import {
  ArrowLeft,
  ArrowRight,
  CheckCircle,
  Database,
  GitBranch,
  Play,
  Plugs,
  ShieldCheck,
  Stack,
  UserCircle,
  WarningCircle,
} from '@phosphor-icons/react'
import { useLayoutEffect, useRef, useState } from 'react'

import { useShikiHighlight } from '../useShikiHighlight'
import './happycontext-blocks.css'

function GoCode({ code }: { code: string }) {
  const lines = useShikiHighlight(code, 'go')
  if (!lines)
    return (
      <pre>
        <code>{code}</code>
      </pre>
    )
  return (
    <pre>
      <code className="shiki-code">
        {lines.map((line, li) => (
          <span key={li} className="line">
            {line.map((tok, ti) => (
              <span key={ti} style={tok.style as React.CSSProperties}>
                {tok.content}
              </span>
            ))}
            {li < lines.length - 1 ? '\n' : null}
          </span>
        ))}
      </code>
    </pre>
  )
}

const traditionalLogging = `logger.Info(ctx, "Request started")
logger.Info(ctx, "user authenticated", "user_id", userID)
logger.Info(ctx, "fetching cart", "cart_id", cartID)
logger.Info(ctx, "processing payment")
logger.Info(ctx, "payment successful")
logger.Info(ctx, "request completed")`

const wideEventCode = `// server/api/checkout.post.ts
func checkout(w http.ResponseWriter, r *http.Request) {
  hc.Add(r.Context(), "user_id", "u_8472", "feature", "checkout")
  hc.Add(r.Context(), "cart_id", "c_42", "items", 3, "total_cents", 9999)
  hc.Add(r.Context(), "payment_method", "card", "payment_status", "success")

  w.WriteHeader(http.StatusOK)
}`

const wideEventOutput = `[INFO] POST /api/checkout (234ms)
  user_id: "u_8472"
  cart_id: "c_42"
  items: 3
  total_cents: 9999
  payment_method: "card"
  payment_status: "success"
  status: 200`

const lifecycleSteps = [
  {
    title: 'Start',
    detail: 'Middleware captures method, path, route, and a request timestamp.',
    fields: ['http.method', 'http.path', 'http.route', 'time'],
  },
  {
    title: 'Enrich',
    detail:
      'Handlers add user, tenant, feature, and domain payload as context arrives.',
    fields: ['user_id', 'tenant_id', 'feature', 'order_id'],
  },
  {
    title: 'Finalize',
    detail:
      'Status, duration, and error fields are computed once the request is done.',
    fields: ['http.status', 'duration_ms', 'error'],
  },
  {
    title: 'Emit',
    detail: 'The adapter writes one canonical event to slog, zap, or zerolog.',
    fields: ['level', 'msg', 'sink'],
  },
]

const wideEventCards = [
  {
    title: 'One Event',
    description:
      'Replace noisy middleware and handler logs with one request-scoped payload.',
    fields: ['request_completed'],
  },
  {
    title: 'Full Context',
    description:
      'Capture HTTP details, user metadata, and business data in one searchable record.',
    fields: ['http', 'user', 'order', 'outcome'],
  },
  {
    title: 'Router Scoped',
    description:
      'Add fields as the request travels through net/http, gin, echo, or fiber.',
    fields: ['std', 'gin', 'echo', 'fiber'],
  },
  {
    title: 'Adapter Friendly',
    description:
      'Keep your existing logger stack and switch only the sink integration.',
    fields: ['slog', 'zap', 'zerolog'],
  },
]

const heroScenarios = [
  {
    id: 'success',
    label: 'Healthy request',
    status: '200 OK',
    tone: 'success',
    summary: 'A complete checkout, captured once.',
    fields: [
      ['http.route', 'POST /checkout'],
      ['user_id', 'u_8472'],
      ['order_id', 'ord_42'],
      ['duration_ms', '184'],
    ],
  },
  {
    id: 'slow',
    label: 'Slow request',
    status: '782 ms',
    tone: 'warning',
    summary: 'The latency signal stays attached to its business context.',
    fields: [
      ['http.route', 'GET /orders/{id}'],
      ['tenant_id', 'team_west'],
      ['db.query_count', '14'],
      ['duration_ms', '782'],
    ],
  },
  {
    id: 'error',
    label: 'Failed request',
    status: '500 ERROR',
    tone: 'error',
    summary: 'The failure keeps every field needed for incident response.',
    fields: [
      ['http.route', 'POST /checkout'],
      ['user_id', 'u_8472'],
      ['payment_attempt', '3'],
      ['error', 'checkout failed'],
    ],
  },
] as const

const routers = ['net/http', 'gin', 'echo', 'fiber v2', 'fiber v3']
const adapters = ['slog', 'zap', 'zerolog']

export function HappyContextHero() {
  const [activeScenario, setActiveScenario] = useState(0)
  const scenario = heroScenarios[activeScenario]

  return (
    <section
      className="hcw-panel hcw-hero hcw-soft-shell not-typography"
      aria-label="happycontext summary"
    >
      <div className="hcw-hero-copy">
        <div className="hcw-kicker">Wide logging for Go services</div>
        <h2 className="hcw-title">
          Read the whole request, not scattered lines.
        </h2>
        <p className="hcw-body">
          Choose a request outcome and inspect the one event that carries its
          HTTP, user, business, and result context.
        </p>
        <div
          className="hcw-choice-row"
          role="group"
          aria-label="Request outcome"
        >
          {heroScenarios.map((item, index) => (
            <button
              key={item.id}
              type="button"
              aria-pressed={activeScenario === index}
              className={activeScenario === index ? 'is-active' : ''}
              onClick={() => setActiveScenario(index)}
            >
              {item.label}
            </button>
          ))}
        </div>
      </div>

      <div
        className={`hcw-event-preview is-${scenario.tone}`}
        aria-live="polite"
      >
        <header>
          <span>
            <span className="hcw-live-dot" />
            Canonical event
          </span>
          <strong>{scenario.status}</strong>
        </header>
        <p>{scenario.summary}</p>
        <dl>
          {scenario.fields.map(([key, value]) => (
            <div key={key}>
              <dt>{key}</dt>
              <dd>{value}</dd>
            </div>
          ))}
        </dl>
        <footer>
          <span>1 request</span>
          <ArrowRight aria-hidden="true" weight="bold" />
          <span>1 queryable event</span>
        </footer>
      </div>
    </section>
  )
}

export function WideEventValueCards() {
  const [active, setActive] = useState(0)
  const card = wideEventCards[active]

  return (
    <section
      className="hcw-context-lens hcw-soft-shell not-typography"
      aria-label="wide event benefits"
    >
      <div
        className="hcw-context-nav"
        role="group"
        aria-label="Wide event value"
      >
        {wideEventCards.map((item, index) => (
          <button
            key={item.title}
            type="button"
            aria-pressed={active === index}
            className={active === index ? 'is-active' : ''}
            onClick={() => setActive(index)}
          >
            <span>{String(index + 1).padStart(2, '0')}</span>
            {item.title}
          </button>
        ))}
      </div>
      <div className="hcw-context-detail" aria-live="polite">
        <div>
          <span className="hcw-kicker">Context lens</span>
          <h3>{card.title}</h3>
          <p>{card.description}</p>
        </div>
        <div className="hcw-field-cloud" aria-label={`${card.title} examples`}>
          {card.fields.map((field) => (
            <code key={field}>{field}</code>
          ))}
        </div>
      </div>
    </section>
  )
}

export function WideEventsBeforeAfter() {
  const [approach, setApproach] = useState<'traditional' | 'wide'>(
    'traditional',
  )
  const [wideView, setWideView] = useState<'code' | 'output'>('output')
  const isWide = approach === 'wide'

  return (
    <section
      className="hcw-compare hcw-soft-shell not-typography"
      aria-label="Traditional logging versus wide events"
    >
      <header className="hcw-compare-header">
        <div>
          <span className="hcw-kicker">Log collapse</span>
          <h3>Six disconnected lines or one complete story?</h3>
        </div>
        <div
          className="hcw-segmented"
          role="group"
          aria-label="Logging approach"
        >
          <button
            type="button"
            aria-pressed={!isWide}
            className={!isWide ? 'is-active' : ''}
            onClick={() => setApproach('traditional')}
          >
            Scattered logs
          </button>
          <button
            type="button"
            aria-pressed={isWide}
            className={isWide ? 'is-active' : ''}
            onClick={() => setApproach('wide')}
          >
            Wide event
          </button>
        </div>
      </header>

      <div className="hcw-collapse-stage" aria-live="polite">
        <div className="hcw-collapse-count">
          <strong>{isWide ? '01' : '06'}</strong>
          <span>{isWide ? 'event to query' : 'lines to correlate'}</span>
        </div>
        <div className="hcw-collapse-code">
          {isWide ? (
            <>
              <div
                className="hcw-mini-tabs"
                role="group"
                aria-label="Wide event view"
              >
                <button
                  type="button"
                  aria-pressed={wideView === 'code'}
                  className={wideView === 'code' ? 'is-active' : ''}
                  onClick={() => setWideView('code')}
                >
                  Go
                </button>
                <button
                  type="button"
                  aria-pressed={wideView === 'output'}
                  className={wideView === 'output' ? 'is-active' : ''}
                  onClick={() => setWideView('output')}
                >
                  Event
                </button>
              </div>
              {wideView === 'code' ? (
                <GoCode code={wideEventCode} />
              ) : (
                <pre>
                  <code>{wideEventOutput}</code>
                </pre>
              )}
            </>
          ) : (
            <GoCode code={traditionalLogging} />
          )}
        </div>
      </div>
      <p className="hcw-insight">
        {isWide
          ? 'Every field shares one request identity, so the event can be filtered and alerted on directly.'
          : 'The request is present, but its story is split across lines that must be correlated later.'}
      </p>
    </section>
  )
}

export function IntegrationsMatrix() {
  const [router, setRouter] = useState(routers[0])
  const [adapter, setAdapter] = useState(adapters[0])

  return (
    <section
      className="hcw-configurator hcw-soft-shell not-typography"
      aria-label="Integrations and adapters matrix"
    >
      <header>
        <div>
          <span className="hcw-kicker">Compatibility configurator</span>
          <h3>Choose a router and keep your logger.</h3>
        </div>
        <span className="hcw-combination-count">15 supported combinations</span>
      </header>
      <div className="hcw-configurator-grid">
        <div>
          <p className="hcw-column-title">
            <GitBranch aria-hidden="true" /> Router
          </p>
          <div
            className="hcw-option-list"
            role="radiogroup"
            aria-label="Router"
          >
            {routers.map((item) => (
              <button
                key={item}
                type="button"
                role="radio"
                aria-checked={router === item}
                className={router === item ? 'is-active' : ''}
                onClick={() => setRouter(item)}
              >
                {item}
                {router === item ? (
                  <CheckCircle aria-hidden="true" weight="fill" />
                ) : null}
              </button>
            ))}
          </div>
        </div>
        <div>
          <p className="hcw-column-title">
            <Plugs aria-hidden="true" /> Logger adapter
          </p>
          <div
            className="hcw-option-list"
            role="radiogroup"
            aria-label="Logger adapter"
          >
            {adapters.map((item) => (
              <button
                key={item}
                type="button"
                role="radio"
                aria-checked={adapter === item}
                className={adapter === item ? 'is-active' : ''}
                onClick={() => setAdapter(item)}
              >
                {item}
                {adapter === item ? (
                  <CheckCircle aria-hidden="true" weight="fill" />
                ) : null}
              </button>
            ))}
          </div>
        </div>
      </div>
      <div className="hcw-config-result" aria-live="polite">
        <div>
          <span>Selected bridge</span>
          <strong>
            {router} → {adapter}
          </strong>
        </div>
        <code>
          integration/{router === 'net/http' ? 'std' : router.replace(' ', '')}{' '}
          + adapter/{adapter}
        </code>
      </div>
    </section>
  )
}

export function RequestLifecycleRail() {
  const [active, setActive] = useState(0)
  const step = lifecycleSteps[active]

  return (
    <section
      className="hcw-lifecycle hcw-soft-shell not-typography"
      aria-label="Request lifecycle"
    >
      <header>
        <span className="hcw-kicker">Request lifecycle</span>
        <h3>Context grows as the request moves.</h3>
      </header>
      <div
        className="hcw-lifecycle-track"
        role="group"
        aria-label="Lifecycle stage"
      >
        {lifecycleSteps.map((item, index) => (
          <button
            key={item.title}
            type="button"
            aria-pressed={active === index}
            className={active === index ? 'is-active' : ''}
            onClick={() => setActive(index)}
          >
            <span>{index + 1}</span>
            {item.title}
          </button>
        ))}
      </div>
      <div className="hcw-lifecycle-focus" aria-live="polite">
        <div>
          <span>
            Stage {active + 1} of {lifecycleSteps.length}
          </span>
          <h4>{step.title}</h4>
          <p>{step.detail}</p>
        </div>
        <div className="hcw-field-cloud">
          {step.fields.map((field) => (
            <code key={field}>{field}</code>
          ))}
        </div>
      </div>
    </section>
  )
}

export function SamplingDecisionPanel() {
  const examples = [
    { label: '500 error', kind: 'error', detail: 'error field present' },
    { label: 'Slow checkout', kind: 'slow', detail: '742 ms on /checkout' },
    { label: 'Admin request', kind: 'protected', detail: '/admin/users' },
    { label: 'Healthy request', kind: 'healthy', detail: '200 OK · 84 ms' },
  ] as const
  const [active, setActive] = useState(0)
  const [keepErrors, setKeepErrors] = useState(true)
  const [keepSlow, setKeepSlow] = useState(true)
  const [keepProtected, setKeepProtected] = useState(true)
  const event = examples[active]
  const policies: Array<{
    label: string
    checked: boolean
    setChecked: (value: boolean) => void
  }> = [
    {
      label: 'Keep errors',
      checked: keepErrors,
      setChecked: setKeepErrors,
    },
    {
      label: 'Keep slow requests',
      checked: keepSlow,
      setChecked: setKeepSlow,
    },
    {
      label: 'Keep protected paths',
      checked: keepProtected,
      setChecked: setKeepProtected,
    },
  ]
  const kept =
    (event.kind === 'error' && keepErrors) ||
    (event.kind === 'slow' && keepSlow) ||
    (event.kind === 'protected' && keepProtected)

  return (
    <section
      className="hcw-policy-tester hcw-soft-shell not-typography"
      aria-label="Sampling behavior"
    >
      <header>
        <div>
          <span className="hcw-kicker">Policy tester</span>
          <h3>Which events survive sampling?</h3>
        </div>
        <span>Healthy traffic samples at 10%</span>
      </header>
      <div className="hcw-policy-grid">
        <div className="hcw-policy-rules">
          {policies.map(({ label, checked, setChecked }) => (
            <button
              key={label}
              type="button"
              role="switch"
              aria-checked={checked}
              className={checked ? 'is-active' : ''}
              onClick={() => setChecked(!checked)}
            >
              <span>{label}</span>
              <span className="hcw-switch-knob" />
            </button>
          ))}
        </div>
        <div
          className="hcw-policy-events"
          role="group"
          aria-label="Example event"
        >
          {examples.map((item, index) => (
            <button
              key={item.label}
              type="button"
              aria-pressed={active === index}
              className={active === index ? 'is-active' : ''}
              onClick={() => setActive(index)}
            >
              {item.label}
            </button>
          ))}
        </div>
      </div>
      <div
        className={`hcw-policy-result ${kept ? 'is-kept' : 'is-sampled'}`}
        aria-live="polite"
      >
        {kept ? (
          <ShieldCheck aria-hidden="true" weight="fill" />
        ) : (
          <WarningCircle aria-hidden="true" weight="fill" />
        )}
        <div>
          <span>{kept ? 'Always keep' : 'Rate sample'}</span>
          <strong>{event.label}</strong>
          <p>
            {event.detail}.{' '}
            {kept
              ? 'A tail rule preserves this event.'
              : 'No keep rule matched, so the base rate applies.'}
          </p>
        </div>
      </div>
    </section>
  )
}

// --- Wide Event Builder Simulator (happycontext Go API) ---

const WIDE_BUILDER_STEPS: Array<{
  title: string
  description: string
  code: string
  event: Record<string, unknown>
}> = [
  {
    title: 'Middleware captures request',
    description:
      'happycontext middleware records method, path, route, and start time.',
    code: `mw := stdhc.Middleware(hc.Config{
  Sink:         sink,
  SamplingRate: 1.0,
  Message:      "request_completed",
})`,
    event: {
      time: '2026-02-09T14:03:12.451Z',
      level: 'INFO',
      msg: 'request_completed',
      'http.method': 'GET',
      'http.path': '/orders/123',
      'http.route': 'GET /orders/{id}',
    },
  },
  {
    title: 'Add user and feature context',
    description:
      'hc.Add accepts key/value pairs; fields are merged into the final event.',
    code: `hc.Add(ctx, "user_id", "u_8472", "feature", "checkout")`,
    event: {
      time: '2026-02-09T14:03:12.451Z',
      level: 'INFO',
      msg: 'request_completed',
      'http.method': 'GET',
      'http.path': '/orders/123',
      'http.route': 'GET /orders/{id}',
      user_id: 'u_8472',
      feature: 'checkout',
    },
  },
  {
    title: 'Add cart and order context',
    description: 'Add more keys as you process the request.',
    code: `hc.Add(ctx, "order_id", "ord_xyz", "total_cents", 15999)
hc.Add(ctx, "coupon_applied", "SAVE20")`,
    event: {
      time: '2026-02-09T14:03:12.451Z',
      level: 'INFO',
      msg: 'request_completed',
      'http.method': 'GET',
      'http.path': '/orders/123',
      'http.route': 'GET /orders/{id}',
      user_id: 'u_8472',
      feature: 'checkout',
      order_id: 'ord_xyz',
      total_cents: 15999,
      coupon_applied: 'SAVE20',
    },
  },
  {
    title: 'Record payment attempt',
    description: 'Add payment or domain fields before responding.',
    code: `hc.Add(ctx, "payment_method", "card", "payment_attempt", 3)
// on error:
hc.Error(ctx, err)`,
    event: {
      time: '2026-02-09T14:03:12.451Z',
      level: 'INFO',
      msg: 'request_completed',
      'http.method': 'GET',
      'http.path': '/orders/123',
      'http.route': 'GET /orders/{id}',
      user_id: 'u_8472',
      feature: 'checkout',
      order_id: 'ord_xyz',
      total_cents: 15999,
      coupon_applied: 'SAVE20',
      payment_method: 'card',
      payment_attempt: 3,
    },
  },
  {
    title: 'On error: hc.Error',
    description:
      'hc.Error marks the request as failed; errors are always kept when sampling.',
    code: `hc.Error(ctx, errors.New("checkout failed"))`,
    event: {
      time: '2026-02-09T14:03:12.451Z',
      level: 'ERROR',
      msg: 'request_completed',
      'http.method': 'GET',
      'http.path': '/orders/123',
      'http.route': 'GET /orders/{id}',
      'http.status': 500,
      duration_ms: 1247,
      user_id: 'u_8472',
      feature: 'checkout',
      order_id: 'ord_xyz',
      total_cents: 15999,
      coupon_applied: 'SAVE20',
      payment_method: 'card',
      payment_attempt: 3,
      error: 'checkout failed',
    },
  },
  {
    title: 'One event at completion',
    description:
      'Middleware writes one canonical event (status, duration_ms, all added fields).',
    code: `// middleware emits after handler returns; Sink receives the final event`,
    event: {
      time: '2026-02-09T14:03:12.451Z',
      level: 'ERROR',
      msg: 'request_completed',
      'http.method': 'GET',
      'http.path': '/orders/123',
      'http.route': 'GET /orders/{id}',
      'http.status': 500,
      duration_ms: 1247,
      user_id: 'u_8472',
      feature: 'checkout',
      order_id: 'ord_xyz',
      total_cents: 15999,
      coupon_applied: 'SAVE20',
      payment_method: 'card',
      payment_attempt: 3,
      error: 'checkout failed',
    },
  },
]

// Lightweight JSON syntax highlighter — no external deps
function HighlightedJson({
  obj,
  newKeys = new Set<string>(),
}: {
  obj: Record<string, unknown>
  newKeys?: Set<string>
}) {
  const lines = JSON.stringify(obj, null, 2).split('\n')
  return (
    <>
      {lines.map((line, i) => {
        const m = line.match(/^(\s*)("(?:[^"\\]|\\.)*")\s*:\s*(.+)$/)
        if (!m) {
          return (
            <span key={i} className="hjson-bracket">
              {line}
              {'\n'}
            </span>
          )
        }
        const [, indent, keyToken, rest] = m
        const hasComma = rest.endsWith(',')
        const valRaw = hasComma ? rest.slice(0, -1) : rest
        const key = keyToken.slice(1, -1)

        let valClass = 'hjson-string'
        if (!valRaw.startsWith('"')) {
          valClass = 'hjson-number'
          if (key === 'http.status' && valRaw === '500')
            valClass = 'hjson-val-error'
        } else {
          if (key === 'level' && valRaw === '"ERROR"')
            valClass = 'hjson-val-error'
          if (key === 'level' && valRaw === '"INFO"')
            valClass = 'hjson-val-info'
          if (key === 'error') valClass = 'hjson-val-error'
        }

        return (
          <span
            key={i}
            className={
              newKeys.has(key) ? 'hjson-line is-new-field' : 'hjson-line'
            }
          >
            {indent}
            <span className="hjson-key">{keyToken}</span>
            <span className="hjson-punct">{': '}</span>
            <span className={valClass}>{valRaw}</span>
            {hasComma && <span className="hjson-punct">{','}</span>}
            {'\n'}
          </span>
        )
      })}
    </>
  )
}

function countFields(obj: Record<string, unknown>): number {
  let n = 0
  for (const v of Object.values(obj)) {
    if (v !== null && typeof v === 'object' && !Array.isArray(v)) {
      n += countFields(v as Record<string, unknown>)
    } else {
      n += 1
    }
  }
  return n
}

export function WideEventBuilderSimulator() {
  const [step, setStep] = useState(0)
  const current = WIDE_BUILDER_STEPS[step]
  const previous = step > 0 ? WIDE_BUILDER_STEPS[step - 1] : null
  const fieldCount = countFields(current.event)
  const isError = current.event.level === 'ERROR'
  const newKeys = new Set(
    Object.keys(current.event).filter(
      (key) => !previous || !(key in previous.event),
    ),
  )
  const stepIcons = [
    Database,
    UserCircle,
    Stack,
    Play,
    WarningCircle,
    CheckCircle,
  ]
  const StepIcon = stepIcons[step]

  return (
    <section
      className="hcw-wide-builder hcw-soft-shell not-typography"
      aria-label="Wide Event Builder Simulator"
    >
      <header className="hcw-wide-builder-heading">
        <div>
          <span className="hcw-kicker">Request event accumulator</span>
          <h3>Watch context become one complete event.</h3>
          <p>
            Move through the request. Each stage adds fields without emitting
            another disconnected log line.
          </p>
        </div>
        <span className="hcw-step-count" aria-live="polite">
          {String(step + 1).padStart(2, '0')} /{' '}
          {String(WIDE_BUILDER_STEPS.length).padStart(2, '0')}
        </span>
      </header>

      <div
        className="hcw-builder-track"
        role="group"
        aria-label="Request stage"
      >
        {WIDE_BUILDER_STEPS.map((item, index) => (
          <button
            key={item.title}
            type="button"
            aria-pressed={step === index}
            className={
              step === index ? 'is-active' : index < step ? 'is-complete' : ''
            }
            onClick={() => setStep(index)}
          >
            <span>
              {index < step ? (
                <CheckCircle aria-hidden="true" weight="fill" />
              ) : (
                index + 1
              )}
            </span>
            <small>{item.title.replace(/^.*?:\s*/, '')}</small>
          </button>
        ))}
      </div>

      <div className="hcw-builder-workspace">
        <article className="hcw-builder-detail">
          <div className="hcw-builder-icon" aria-hidden="true">
            <StepIcon weight="duotone" />
          </div>
          <span className="hcw-kicker">Stage {step + 1}</span>
          <h4>{current.title}</h4>
          <p>{current.description}</p>
          <div className="hcw-wide-builder-code">
            <GoCode code={current.code} />
          </div>
          {newKeys.size > 0 ? (
            <div className="hcw-added-fields">
              <span>Added now</span>
              <div>
                {[...newKeys].map((key) => (
                  <code key={key}>{key}</code>
                ))}
              </div>
            </div>
          ) : null}
        </article>

        <article className="hcw-wide-builder-live">
          <header className={isError ? 'is-error' : ''}>
            <span>
              <span className="hcw-live-dot" />
              Live event
            </span>
            <strong>{fieldCount} fields</strong>
          </header>
          <div className="hcw-event-growth">
            <span
              style={{
                width: `${((step + 1) / WIDE_BUILDER_STEPS.length) * 100}%`,
              }}
            />
          </div>
          <pre>
            <code>
              <HighlightedJson obj={current.event} newKeys={newKeys} />
            </code>
          </pre>
          <footer>
            <span>
              {isError
                ? 'Tail rule keeps this event'
                : 'Still one in-flight event'}
            </span>
            {isError ? (
              <ShieldCheck aria-hidden="true" weight="fill" />
            ) : (
              <CheckCircle aria-hidden="true" weight="fill" />
            )}
          </footer>
        </article>
      </div>

      <footer className="hcw-wide-builder-footer">
        <p>
          <strong>{fieldCount}</strong> fields, <strong>1</strong> event,{' '}
          <strong>0</strong> correlation work.
        </p>
        <div
          className="hcw-wide-builder-nav"
          role="group"
          aria-label="Step navigation"
        >
          <button
            type="button"
            onClick={() => setStep((value) => Math.max(0, value - 1))}
            disabled={step === 0}
            aria-label="Previous step"
          >
            <ArrowLeft aria-hidden="true" weight="bold" />
            Previous
          </button>
          <button
            type="button"
            onClick={() =>
              setStep((value) =>
                Math.min(WIDE_BUILDER_STEPS.length - 1, value + 1),
              )
            }
            disabled={step === WIDE_BUILDER_STEPS.length - 1}
            aria-label="Next step"
          >
            Next
            <ArrowRight aria-hidden="true" weight="bold" />
          </button>
        </div>
      </footer>
    </section>
  )
}

// --- The Sampling Trap ---

const TOTAL_EVENTS = 2_000
// success 70% · warn 15% · slow 10% · error 5%
const SUCCESS_PCT = 0.7
const WARN_PCT = 0.15
const SLOW_PCT = 0.1

type EventType = 'success' | 'warn' | 'slow' | 'error'

function buildEventStream(): EventType[] {
  const out: EventType[] = []
  let idx = 0
  const nSuccess = Math.floor(TOTAL_EVENTS * SUCCESS_PCT)
  const nWarn = Math.floor(TOTAL_EVENTS * WARN_PCT)
  const nSlow = Math.floor(TOTAL_EVENTS * SLOW_PCT)
  const nError = TOTAL_EVENTS - nSuccess - nWarn - nSlow
  for (let i = 0; i < nSuccess; i++) out.push('success')
  for (let i = 0; i < nWarn; i++) out.push('warn')
  for (let i = 0; i < nSlow; i++) out.push('slow')
  for (let i = 0; i < nError; i++) out.push('error')
  const rng = (): number => {
    const x = Math.sin(42 * 9999 + idx++) * 10000
    return x - Math.floor(x)
  }
  for (let i = out.length - 1; i > 0; i--) {
    const j = Math.floor(rng() * (i + 1))
    ;[out[i], out[j]] = [out[j], out[i]]
  }
  return out
}

const EVENT_STREAM = buildEventStream()

function sampleRandom(rate: number): boolean[] {
  // xorshift32 — sequential RNG produces natural clusters, not a uniform grid
  let s = 0xdeadbeef
  return EVENT_STREAM.map(() => {
    s ^= s << 13
    s ^= s >>> 17
    s ^= s << 5
    return (s >>> 0) / 0xffffffff < rate
  })
}

function computeTailSampling(rate: number): boolean[] {
  let s = 12345
  return EVENT_STREAM.map((t) => {
    if (t === 'error' || t === 'slow') return true
    s = (s * 9301 + 49297) % 233280
    return s / 233280 < rate
  })
}

// Matches --color-success / --color-warning / --color-error from global.css
const DOT_COLORS: Record<EventType, string> = {
  success: '#22c55e',
  warn: '#f59e0b',
  slow: '#f97316',
  error: '#ef4444',
}

const CELL_SIZE = 10 // visible box size in px
const CELL_GAP = 2 // gap between boxes in px
const CELL_STRIDE = CELL_SIZE + CELL_GAP
const MAX_ROWS = 20 // cap so narrow screens stay ~240px tall, not 800px

function drawEventGrid(
  canvas: HTMLCanvasElement,
  stream: EventType[],
  kept: boolean[],
  containerW: number,
) {
  const dpr = Math.min(2, window.devicePixelRatio ?? 1)
  const cols = Math.max(1, Math.floor((containerW + CELL_GAP) / CELL_STRIDE))
  // Limit visible events to MAX_ROWS rows — keeps chart height consistent
  const visibleCount = Math.min(stream.length, cols * MAX_ROWS)
  const rows = Math.ceil(visibleCount / cols)
  const W = cols * CELL_STRIDE - CELL_GAP
  const H = rows * CELL_STRIDE - CELL_GAP

  canvas.width = W * dpr
  canvas.height = H * dpr
  canvas.style.width = `${W}px`
  canvas.style.height = `${H}px`

  const ctx = canvas.getContext('2d')
  if (!ctx) return

  const stride = CELL_STRIDE * dpr
  const size = CELL_SIZE * dpr
  const radius = 2 * dpr

  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      const i = row * cols + col
      if (i >= visibleCount) break
      ctx.fillStyle = DOT_COLORS[stream[i]]
      ctx.globalAlpha = kept[i] ? 1 : 0.15
      const x = col * stride
      const y = row * stride
      ctx.beginPath()
      ctx.roundRect(x, y, size, size, radius)
      ctx.fill()
    }
  }
  ctx.globalAlpha = 1
}

export function SamplingTrap() {
  const [sampleRate, setSampleRate] = useState(10)
  const [mode, setMode] = useState<'random' | 'tail'>('random')
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const wrapRef = useRef<HTMLDivElement>(null)
  const [containerW, setContainerW] = useState(0)

  const rate = sampleRate / 100
  const kept =
    mode === 'random' ? sampleRandom(rate) : computeTailSampling(rate)

  const keptCount = kept.filter(Boolean).length
  const keptSuccess = EVENT_STREAM.filter(
    (t, i) => t === 'success' && kept[i],
  ).length
  const keptWarn = EVENT_STREAM.filter((t, i) => t === 'warn' && kept[i]).length
  const keptSlow = EVENT_STREAM.filter((t, i) => t === 'slow' && kept[i]).length
  const keptErrors = EVENT_STREAM.filter(
    (t, i) => t === 'error' && kept[i],
  ).length
  const missChance = Math.round((1 - rate) * 100)

  useLayoutEffect(() => {
    const el = wrapRef.current
    if (!el) return
    // contentRect excludes padding — matches the inner drawing area
    const ro = new ResizeObserver((entries) => {
      setContainerW(Math.floor(entries[0].contentRect.width))
    })
    ro.observe(el)
    setContainerW(Math.floor(el.clientWidth - 21)) // 2×0.65rem padding ≈ 21px
    return () => ro.disconnect()
  }, [])

  useLayoutEffect(() => {
    const canvas = canvasRef.current
    if (!canvas || containerW === 0) return
    drawEventGrid(canvas, EVENT_STREAM, kept, containerW)
  }, [sampleRate, mode, containerW])

  return (
    <section
      className="hcw-sampling-trap not-typography"
      aria-label="The Sampling Trap"
    >
      <h3 className="hcw-section-title">The Sampling Trap</h3>
      <p className="hcw-sampling-trap-desc">
        Random sampling drops events blindly. Tail-based sampling keeps all
        errors and slow requests while sampling success.
      </p>

      <div className="hcw-st-chart-header">
        <span className="hcw-st-stream-label">
          Event Stream ({TOTAL_EVENTS.toLocaleString()}&thinsp;events)
        </span>
        <div className="hcw-st-legend">
          <span className="hcw-st-legend-item">
            <span className="hcw-st-dot hcw-dot-success" />
            Success (70%)
          </span>
          <span className="hcw-st-legend-item">
            <span className="hcw-st-dot hcw-dot-warn" />
            Warn (15%)
          </span>
          <span className="hcw-st-legend-item">
            <span className="hcw-st-dot hcw-dot-slow" />
            Slow (10%)
          </span>
          <span className="hcw-st-legend-item">
            <span className="hcw-st-dot hcw-dot-error" />
            Error (5%)
          </span>
        </div>
      </div>

      <div
        ref={wrapRef}
        className="hcw-st-chart-wrap"
        role="img"
        aria-label={`Event stream: ${keptCount} of ${TOTAL_EVENTS} events kept at ${sampleRate}% ${mode} sampling`}
      >
        <canvas ref={canvasRef} className="hcw-st-canvas" />
      </div>

      <div className="hcw-st-controls">
        <div className="hcw-st-slider-row">
          <label htmlFor="hcw-sample-rate" className="hcw-st-slider-label">
            Sample Rate
          </label>
          <input
            id="hcw-sample-rate"
            type="range"
            className="hcw-st-slider"
            min={1}
            max={100}
            value={sampleRate}
            onChange={(e) => setSampleRate(Number(e.target.value))}
            aria-valuemin={1}
            aria-valuemax={100}
            aria-valuenow={sampleRate}
            aria-valuetext={`${sampleRate}%`}
          />
          <span className="hcw-st-rate-value">{sampleRate}%</span>
        </div>
        <div
          className="hcw-st-mode-buttons"
          role="group"
          aria-label="Sampling type"
        >
          <button
            type="button"
            className={mode === 'random' ? 'is-active' : ''}
            onClick={() => setMode('random')}
          >
            Random Sampling
          </button>
          <button
            type="button"
            className={mode === 'tail' ? 'is-active' : ''}
            onClick={() => setMode('tail')}
          >
            Tail Sampling
          </button>
        </div>
      </div>

      <div className="hcw-st-table-wrap">
        <table className="hcw-st-table">
          <thead>
            <tr>
              <th>EVENTS KEPT</th>
              <th>SUCCESS</th>
              <th>WARN</th>
              <th>SLOW</th>
              <th>ERRORS</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td>{keptCount}</td>
              <td className="hcw-st-success">{keptSuccess}</td>
              <td className="hcw-st-warn">{keptWarn}</td>
              <td className="hcw-st-slow">{keptSlow}</td>
              <td className="hcw-st-error">{keptErrors}</td>
            </tr>
          </tbody>
        </table>
      </div>

      <div className="hcw-st-warning">
        {mode === 'random' ? (
          <>
            At {sampleRate}% random sampling, you have a{' '}
            <strong>{missChance}% chance</strong> of missing any specific error.
          </>
        ) : (
          <>
            With tail sampling (e.g. <code>hc.KeepErrors()</code>,{' '}
            <code>hc.KeepSlowerThan(...)</code>,{' '}
            <code>hc.RateSampler(...)</code>
            ), all errors and slow requests are kept; only success traffic is
            sampled at {sampleRate}%.
          </>
        )}
      </div>
    </section>
  )
}
