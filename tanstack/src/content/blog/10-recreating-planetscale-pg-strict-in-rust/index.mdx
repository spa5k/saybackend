...
<InteractiveDiagram
  title="Postgres query pipeline and hook points"
  variant="journey"
  code={`
graph TB
...
<InteractiveDiagram
  title="Stage 0 · Executor hook proof of concept"
  variant="journey"
  code={`
graph LR
...
<InteractiveDiagram
  title="Stage 1 · Tree-sitter attempt"
  variant="journey"
  code={`
graph LR
...
<InteractiveDiagram
  title="Stage 2 · External sqlparser approach"
  variant="journey"
  code={`
graph LR
...
<InteractiveDiagram
  title="Stage 3 · Postgres parser at the executor phase"
  variant="journey"
  code={`
graph LR
...
<InteractiveDiagram
  title="Stage 4 · Parse-time enforcement"
  variant="journey"
  code={`
graph LR
...
<PgStrictPlayground client:load />
...
<InteractiveDiagram
  title="pg_strict extension architecture"
...
<GitHubRepoCard
  ...
  title="spa5k/pg_strict"
  description="A Postgres strictness layer that blocks UPDATE/DELETE without a WHERE clause."
/>

If this extension helps your team avoid accidental full-table updates or deletes, a GitHub star helps a lot.
