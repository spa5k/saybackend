...
faqs:
  - question: "Can you really use Next.js inside Electron?"
    answer: "Yes. You can bundle an Electron shell and run a Next.js app inside it. The key is deciding whether the Next.js server runs locally (packaged) or the Electron app points to a remote deployment."
  - question: "Do React Server Components work in Electron?"
    answer: "They can, but the constraints are the same as the web: you need a compatible Next.js runtime and you have to be careful about what runs on the server vs. in the renderer process."
  - question: "What’s the biggest gotcha with Electron + Next.js?"
    answer: "Build/runtime boundaries. Electron’s main process, renderer process, and the Next.js server all have different constraints—so keeping APIs, file access, and routing clean matters."
  - question: "Should I use a custom protocol or file:// URLs?"
    answer: "Usually you’ll use a local server URL during dev. In production, many setups prefer a packaged server or a custom protocol to avoid file path quirks and to keep routing predictable."
  - question: "How do I ship updates safely?"
    answer: "Treat the Electron shell and the Next.js app as separate artifacts. Use a proven auto-update mechanism for the shell, and version/deploy the app consistently so you don’t mix incompatible builds."
...
Building an app with Electron and Next.js gets harder once React Server
Components (RSCs) enter the picture. A static export loses much of the dynamic
and server-rendered behavior we want from Next.js.

Running Vite in Electron and Next.js for SSR is another option, but sharing the
same client and server components quickly becomes awkward.

The setup below keeps Next.js as the renderer, including RSCs, while Electron
handles the native APIs.
...
Server Components run on the server and send an RSC payload to the client. They
can access server-side data directly without adding that component's code to the
client JavaScript bundle.

For Electron, the important part is keeping that Next.js server available rather
than reducing the application to a static export.
...
The result has a server component, a client component, and a bridge to Electron's
native APIs.
...
<GitHubRepoCard
  repo="spa5k/nextjs_approuter_electron"
  title="spa5k/nextjs_approuter_electron"
  description="Companion repo for this post: Next.js App Router + Electron (with Server Components)."
/>
