...
![NextJS-electron](/images/dockerfile-golang-og.png)
...
However, this results in you losing large amount of functionality and dynamic content behaviour that you would be getting with SSR, or React Server Components (RSCs).
...
So, let's see how we can build an app with Next.js and Electron with SSR and Server Components support.
...
<Notation type="bracket">
  This is just the primer, but you can read more about it
  [here](https://react.dev/blog/2020/12/21/data-fetching-with-react-server-components).
</Notation>
...
> Preferable, create a Makefile to run the commands, as it gets too complex to run them individually.
...
const webDir = join(dirname(dirname(app.getAppPath())), "web"); // This is where the Next.js build is stored.
...
And that's it! You have successfully built an app with Next.js and Electron with SSR and Server Components support.
...
> NOTE: This is only set it output it as a directory, you can change it to a different format, like .exe, .dmg, etc. by changing the `--dir` whatever you want accordingly.
