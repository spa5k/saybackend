...
flowchart TD
    ...
    B -->|Function 1| C[Generate UUIDv4 Base]
    B -->|Function 2| D[Generate UUIDv4 Base]  
    B -->|Function 3| E[Extract Current Timestamp]
...
graph TB
    subgraph "UUID Family"
        UUIDv4["🔴 UUIDv4<br/>(Baseline)<br/>• Pure random<br/>• No time info<br/>• PostgreSQL native"]
        
        subgraph "UUIDv7 Implementations"
            UUIDv7_PL["🔵 UUIDv7 (PL/pgSQL)<br/>• Overlay method<br/>• Best single-thread<br/>• Readable code"]
            UUIDv7_SQL["🟢 UUIDv7 (Pure SQL)<br/>• Bit operations<br/>• No PL/pgSQL overhead<br/>• Balanced performance"]
            UUIDv7_SUB["🟡 UUIDv7 (Sub-ms)<br/>• Custom precision<br/>• Manual construction<br/>• Best time ordering"]
        end
    end
    
    subgraph "Alternative Formats"
        ULID["🟠 ULID<br/>• Base32 encoded<br/>• Human readable<br/>• Lexicographic sort<br/>• Compact storage"]
        TypeID["🟣 TypeID<br/>• Prefixed identifiers<br/>• Type safety<br/>• Based on UUIDv7<br/>• Self-documenting"]
    end
    
    subgraph "Performance Characteristics"
        PERF_SINGLE["📈 Single-threaded<br/>UUIDv7 (PL/pgSQL) fastest<br/>72.3 μs"]
        PERF_CONCURRENT["⚡ Concurrent<br/>UUIDv4 fastest<br/>29,492 IDs/sec"]
        PERF_STORAGE["💾 Storage<br/>ULID most compact<br/>26 bytes text"]
    end
...
```mermaid
flowchart TD
    START[Choose ID Generation Method] --> Q1{Primary Requirement?}
    
    Q1 -->|Maximum Concurrent Performance| UUIDv4_CHOICE[🔴 UUIDv4<br/>29,492 IDs/sec<br/>Battle-tested]
    Q1 -->|Time Ordering| Q2{Need Sub-millisecond Precision?}
    Q1 -->|Human Readability| ULID_CHOICE[🟠 ULID<br/>Base32 encoded<br/>Lexicographic sort]
    Q1 -->|Type Safety| TYPEID_CHOICE[🟣 TypeID<br/>Prefixed identifiers<br/>Self-documenting]
    
    Q2 -->|Yes| Q3{PostgreSQL Version?}
    Q2 -->|No| Q4{Performance Priority?}
    
    Q3 -->|18+| PG18_CHOICE[🟢 PostgreSQL 18<br/>native uuidv7()<br/>C-level performance]
    Q3 -->|< 18| SUBMS_CHOICE[🟡 UUIDv7 Sub-ms<br/>Custom precision<br/>Best time ordering]
    
    Q4 -->|Single-threaded Speed| PLPGSQL_CHOICE[🔵 UUIDv7 PL/pgSQL<br/>72.3 μs<br/>16% faster than UUIDv4]
    Q4 -->|Balanced Approach| SQL_CHOICE[🟢 UUIDv7 Pure SQL<br/>No PL/pgSQL overhead<br/>Good all-around]
    
    UUIDv4_CHOICE --> RESULT[✅ Implementation Selected]
    ULID_CHOICE --> RESULT
...
graph LR
    subgraph "🏁 Performance Champions"
        SINGLE["🥇 Single-threaded<br/>UUIDv7 (PL/pgSQL)<br/>72.3 μs<br/>16% faster than UUIDv4"]
        CONCURRENT["🥇 Concurrent<br/>UUIDv4<br/>29,492 IDs/sec<br/>38% faster than best UUIDv7"]
        STORAGE["🥇 Storage Efficient<br/>ULID<br/>26 bytes<br/>Most compact text"]
    end
    
    subgraph "🎯 Specialized Features"
        READABLE["👁️ Human Readable<br/>ULID<br/>Base32 encoding<br/>No special characters"]
        TYPESAFE["🛡️ Type Safe<br/>TypeID<br/>Prefixed identifiers<br/>Self-documenting"]
        PRECISION["⏱️ Time Precision<br/>UUIDv7 (Sub-ms)<br/>Sub-millisecond<br/>Best ordering"]
    end
    
    subgraph "🔮 Future Ready"
        NATIVE["🚀 PostgreSQL 18+<br/>Native uuidv7()<br/>C-level implementation<br/>Expected performance leader"]
    end
