...
After the local rules were working, I copied all 15 rules from [`dmmulroy/anti-slop`](https://github.com/dmmulroy/anti-slop/commit/6d53855) into `tools/oxlint/anti-slop/`. I pinned commit `6d53855` and included the MIT license. Our repository change landed in `05c03371`, so the exact source is easy to review later.
...
Useful references are the [Oxlint config reference](https://oxc.rs/docs/guide/usage/linter/config-file-reference) and [type-aware linting guide](https://oxc.rs/docs/guide/usage/linter/type-aware.html). The [CLI reference](https://oxc.rs/docs/guide/usage/linter/cli) covers warning thresholds, `--deny-warnings`, formats, and config discovery.
