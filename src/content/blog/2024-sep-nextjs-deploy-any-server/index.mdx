---
title: "Deploy Next.js Without Vercel: Self-Hosting Production Guide"
description: "Deploy a Next.js app without Vercel using standalone output, Docker, and a production-ready Node server that can run on any VM, container host, or cloud."
date: "2024-09-06"
updated: "2025-04-03"
oldPath: "04-deploy-nextjs-to-production-without-vercel"
tags:
  - "nextjs"
  - "ssr"
  - "rsc"
  ...
  - "deployment"
faqs:
  - question: "Can I deploy Next.js without Vercel?"
    answer: "Yes. You can ship Next.js as a Dockerized Node server (or a platform-native build) and run it anywhere you can run containers or Node—VMs, Kubernetes, or your own servers."
  - question: "What is the Next.js standalone output?"
    answer: "Standalone output packages the minimal server code and dependencies needed to run your app. It’s a good fit for Docker images because it keeps runtime artifacts smaller and more predictable."
  - question: "How do I handle environment variables in production?"
    answer: "Keep build-time and runtime variables separate. Public variables are baked into the client bundle at build time, while secrets (API keys, database URLs) must be provided at runtime via your host or container environment."
  - question: "Do I need Kubernetes for a production Next.js deployment?"
    answer: "No. Kubernetes helps with orchestration and rollouts at scale, but a single VM (or a small cluster behind a reverse proxy) is often enough for many production apps—especially early on."
  - question: "How do I do zero-downtime deploys?"
    answer: "Use a rolling strategy: start new instances, verify health, then drain old ones behind a load balancer or reverse proxy. In Kubernetes, use readiness probes + rolling updates; on VMs, use blue/green or a process manager with graceful reloads."
...
<Callout type="info">
  **Direct answer:** Yes, you can deploy Next.js without Vercel. The most
  portable path is `output: "standalone"`, a small Docker image, runtime
  environment variables, and a regular Node process behind your reverse proxy or
  load balancer.
</Callout>

Updated for Next.js 15. Production deployments usually start with Vercel, then
move toward providers such as Netlify or Firebase. They are convenient, but
their cost and platform limits can become a problem.

A wrong serverless configuration can also lead to a large bill, like this
[story](https://www.youtube.com/watch?v=SCIfWhAheVw) where a solo developer got a
$100,000 Vercel bill after her app went viral.

So, what can we do?

The production deployment is not obvious from inspecting `.next`, but Next.js
can emit a standalone Node server. It runs without serverless functions or a
Vercel runtime.

This post packages that standalone output into a small Docker image.

If you are comparing production deployment patterns, the related [Go Dockerfile optimization guide](/blog/2024-jun-golang-dockerfile-optimized/) covers image-size tradeoffs, and the [Next.js Electron Server Components guide](/blog/2024-aug-nextjs-electron-server-components/) shows where standalone server output matters outside a normal hosted web app.
...
A health check serves both container orchestrators and load balancers. Docker
can also use it to decide when the container is ready to receive traffic.
...
The result is a standalone Next.js server in a small Docker image, with reusable
build layers and a cache handler. It can run on Coolify, Docker, Kubernetes, or
any host that can run a Node container.
