...
Claude Code and Conductor expect Anthropic-style configuration. CLIProxyAPI can sit in the middle and expose a local Anthropic-compatible endpoint while routing requests to Codex-backed OpenAI models.

The setup is:

- **Claude Code** points at CLIProxyAPI
- **Conductor** keeps using its Claude Code runner
...
Once the aliases are in place, both Claude Code and Conductor can use the same local proxy path:

```text
Conductor -> Claude Code -> CLIProxyAPI -> Codex-backed models
```
