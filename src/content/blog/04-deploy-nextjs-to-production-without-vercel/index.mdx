...
<Callout type="warn">
  ...
  had failed. Now we are only copying the package.json and pnpm-lock.yaml in the
  first stage, and the rest of the files in the second stage, so the caching
  will work.
</Callout>
2. If you are running this in Github Actions, elsewhere, you can build the
...
<Callout type="info">
  We are using docker image node:22-slim, because it's based on Debian, and the
...
<Callout type="info">
  ...
  please consider me, I'm available on my email <a href="mailto:admin@saybackend.com">admin@saybackend.com</a>
</Callout>