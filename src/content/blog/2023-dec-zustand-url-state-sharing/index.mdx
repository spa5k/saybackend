...
This stores a Zustand counter in the URL hash. Copy the link, reload it, or open
it in another tab, and the store comes back in the same state.
...
```
https://saybackend.com#counter=%7B%22state%22%3A%7B%22count%22%3A1%7D%2C%22version%22%3A0%7D
```

Reloading or sharing the URL now restores the same counter state.

## Conclusion

The custom `StateStorage` adapter is the whole trick: Zustand's `persist`
middleware writes JSON to the hash instead of `localStorage`. Keep the stored
slice small, because the URL is now part of your application's state format.
