...
<Picture
  src={OgImage}
  alt="Factory Droid with Claude Max and OpenAI Codex subscriptions via CLIProxyAPI"
  inferSize
  formats={["avif", "webp"]}
/>
...
<Callout type="info">
  Both Claude and Codex work through the same proxy at the same time. Switch
  models in Factory with `/model` -- no restart needed.
</Callout>
...
<Callout type="info">
  Use `~/.factory/settings.json` with camelCase fields (`customModels`,
  `baseUrl`, `apiKey`). The legacy `~/.factory/config.json` with snake_case
  fields (`custom_models`, `base_url`, `api_key`) still works but takes lower
  priority and doesn't support environment variable expansion. Remove
  `config.json` if you have both.
</Callout>
